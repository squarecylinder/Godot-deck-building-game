...
func _would_cross_existing_path(floor_number: int, column: int, room: Room) -> bool:
	...
	# Can't cross in left direction if left neighbor goes to the right
	if left_neighbor and room.column < column:
...
func _setup_boss_room() -> void:
	...
	for column in MAP_WIDTH:
		var current_room = map_data[FLOORS - 2][column] as Room
